...
预览和应用均指定同一版本；省略 --version 会使用 activeVersion。预览返回 revision/version，应用使用该 revision。文件、映射或目标版本日期/资源变化后必须重新预览。若源成本列未包含启用的 LOCAL/ARP 3%，请只映射实际人天、不映射基础成本列；映射成本列时须与平台年度成本一致。
