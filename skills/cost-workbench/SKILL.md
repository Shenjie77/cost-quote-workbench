...
Start with `npm run --silent cost-cli -- system capabilities`. Run `schema show` for the data being changed; the schema is authoritative. Use [narrow resources](references/resources.md) for everyday edits; load [operations](references/operations.md) only for imports, Excel mapping, review evidence, or legacy backup commands.

Start with `project list`, then read only the needed `project`, `cost`, `masterdata`, `cpq`, `quote`, `ssr`, or `boq` resource. Do not use full `workspace get/save` for routine edits. Pass exact returned revision to the narrow update; on conflict read that resource again and reapply the intended change. Page collections, filter by ID/query, and do not combine pages with different revisions. Use `--compact` on existing CPQ, import, SSR and maintenance mutations. New resource updates already return compact receipts. Cost validation/calculation/export accept `--project-id` and `--version`, without a snapshot request.

Check `costLockReason` before cost or RE updates. Completed DRB (approval with conditions closed, or explicitly completed DRB workflow/review) and finalized costs (`Confirmed`, or completed cost-baseline approval) lock the project’s cost versions and RE catalogue. Do not change states, remove evidence, create a replacement version or clear a lock to bypass this rule. Quote/T&C work and exports may continue. The repository retains the lock across later workflow-label changes. SSR submission snapshots, CPQ archives and maintenance archives are immutable.
...
Project removal requires an actual user deletion request. Use `project delete` with the displayed project ID and current revision; it hides the project and retains local recovery data. `project list --deleted` and `project restore` recover it explicitly. Never interpret missing/deleted data as permission to recreate a project.
