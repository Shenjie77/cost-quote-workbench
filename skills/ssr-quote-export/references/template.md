...
客户报价设 purpose:"quote"。必需映射 quote.number、quote.quoteBeforeTax、quote.gstAmount、quote.quoteAfterTax、quote.paymentTerms、quote.validityDays 和非空 quote.termsAndConditions。可补 project.*、proposalNumber、quote.documentTitle/documentTitleZh/paymentTermsZh。

存在 included 假设时必须映射 assumptions 表的 text，可用 id/text/textZh。quoteLines 提供 scope/amount 汇总行。客户文件禁止 cost.* 标量、costRows 数据集、内部成本/费率/毛利；原表保留内容也要检查。输出不代表公司审批或发送客户。
