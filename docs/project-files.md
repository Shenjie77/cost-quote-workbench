...
- 在现有 **Steps** 列表选择流程节点，再将文件拖到该节点的 **Documents** 卡片；也可点击 **Drop files here** 区域选择文件，无需再次选择节点。卡片显示实际保存目录：开启节点文件夹时为 `workflow / <节点名称>`，否则为 `workflow`。上传文件归属于所显示的项目、流程轮次和节点。
...
**Open Folder** 可打开已保存的全局归档根目录、当前项目归档目录，或每条文档实际所在的文件夹。打开位置在运行本地 API 的电脑上：Windows 使用 Explorer，macOS 使用 Finder，Linux 使用桌面文件管理器。尚未保存的路径输入不会作为打开目标；打开操作不迁移文档。

项目及步骤文档列表的每条记录提供 **Download / Open Folder / Delete**。删除先确认文件名，成功后刷新文档列表，其他文档和项目成本、报价、流程状态保持不变。删除按项目和文件 ID 定位；失败时保留原件或已验证的恢复副本，后续访问归档时重试恢复。删除后的旧上传请求不能悄悄重建该文档，重新选择文件可发起新上传。
...
流程模板的 **Create Folder** 默认开启，创建项目时会保存各步骤的目录选择。关闭的步骤不建立单独文件夹，其文档进入 `workflow` 根目录，步骤和成本轮次仍可筛选查询。之后切换模板开关、给旧项目增加步骤或创建新成本轮次，都不会改变该项目已保存的目录选择。旧数据库升级从已记录的步骤目录保存策略，不补建或删除原来的目录。

逻辑类别保留兼容：`cost/source` 存入 `cost`，`quote/cpq/maintenance` 存入 `quotation`，`general/backup` 存入 `workflow` 根目录，节点文档按项目保存的目录选择存入 `workflow/<节点名称>` 或 `workflow` 根目录。CLI 的 `--category` 参数仍使用这些原有类别，不使用 `quotation` 作为类别值。
...
在 **Project List → Edit → Tags** 输入新标签，或选择已有标签，按 Enter / Add tag 添加，再点击 **Save Project Info** 保存。每个项目可分配多个标签；点击标签旁的 × 可移除该项目的关联。标签忽略大小写和重复空格，支持中文。每个标签最多 40 字符，每个项目最多 30 个标签。

项目列表显示标签；点击标签或使用 **All tags** 下拉框筛选同类项目。表头搜索支持标签，也可与项目名称、Proposal Number 或流程状态组合搜索；选择搜索结果仍保持当前页面。

标签保存于工作区顶层的可选 `projectTags` 数组，并投影到项目索引。旧项目默认无标签；修改标签不会改写成本版本、历史报价或商业依据指纹。标签建议来自当前项目组合中已使用的标签。
