...
新建项目成功时自动建立独立文件夹，页面和 CLI 使用同一逻辑。已有项目首次使用归档时建立文件夹。文件夹使用项目名称（最长 40 个 UTF-16 单元），同名时添加短序号，例如 `Document Project (2)`，不再附加项目 ID 或 UUID；修改项目名称不会移动已有文件。
...
磁盘文件使用清理后的原文件名，保留扩展名，最长 80 个 UTF-16 单元并限制 UTF-8 字节长度；同名使用 `(2)`、`(3)` 等短序号，列表和下载保留原文件名。同名上传分别保留，不覆盖历史文件；上传失败可以重试。同一上传请求重试不会重复入库。SQLite 保存项目归属、逻辑类别、版本、节点、时间、大小、校验值和相对路径，文件内容保存在文件夹中。
...
在 **Master Data → Project Tags** 新增、编辑或停用全局标签，点击 **Save this tab** 保存。支持导出固定 Excel 模板和批量导入。项目编辑的 **Tags** 区只提供已保存启用标签的搜索及多选，不再允许在项目中新增标签；取消勾选可移除关联，再点击 **Save Project Info** 保存。每个标签最多 40 字符，每个项目最多 30 个标签，标签名称不区分大小写且必须唯一。

项目列表显示标签；点击标签或使用 **All tags** 下拉框筛选同类项目。表头搜索支持标签，也可与项目名称、Proposal Number 或流程状态组合搜索；选择搜索结果仍保持当前页面。

标签保存于工作区顶层的可选 `projectTags` 数组，并投影到项目索引。旧项目默认无标签；修改标签不会改写成本版本、历史报价或商业依据指纹。升级时已有标签一次性汇入全局目录。停用、改名或删除主数据标签不改写现有项目的分类：旧标签仍显示，可取消关联，但不能作为新标签再次分配。标签目录不参与新项目的成本和商业快照。

首次访问已有项目归档时，会识别旧系统生成的 UUID 目录和文件名前缀，通过复制、校验、提交映射和清理旧副本缩短路径。自定义目录不自动改名；重复文件即使内容相同也保持独立记录。迁移后文档 ID 和下载入口不变，成本及报价数据不变。
