...
### Project documents and archive location

`files` commands operate directly on the document archive. They do not return
or rewrite the workspace, cost inputs, workflow progress, or project revision.
The global archive settings have their own revision and require no project ID.

```bash
cost-cli files settings
cost-cli files settings --root /absolute/archive/folder --expected-revision 1
cost-cli files list --project-id PRJ-2026-001
cost-cli files list --project-id PRJ-2026-001 --node-code DTRB --version V1
cost-cli files upload --project-id PRJ-2026-001 \
  --node-code DTRB --version V1 --input /absolute/TD-plan.xlsx --compact
cost-cli files upload --project-id PRJ-2026-001 \
  --category general --input /absolute/customer-scope.pdf
cost-cli files download --project-id PRJ-2026-001 \
  --file-id FILE_ID --output /absolute/downloads/TD-plan.xlsx
```

Read the actual settings revision before changing the root; the example `1`
only applies to an unconfigured archive. Changing the configured root controls
new project folders. Existing projects retain their archive locations so links
and previous files remain valid. The service creates a project folder when the
project is created. New archived files use three top-level folders:

- `workflow`: general documents and backups; node documents use a readable
  `workflow/<node name>` subfolder, without a workflow-version directory.
- `cost`: logical `cost` and `source` categories, including imported workbooks
  and original templates.
- `quotation`: logical `quote`, `cpq` and `maintenance` categories.

The logical categories and `nodeCode`/`versionCode` metadata remain unchanged.
Continue using their existing values with `--category`, `--node-code` and
`--version`; `quotation` is a folder name, not a valid category argument.
Use file metadata to identify the workflow round rather than its disk path.
Readable node folders preserve spaces and replace unsafe path punctuation.
Layout migration uses current node names while retaining captured file metadata;
existing indexed files and unindexed regular files are preserved without overwrite.

Upload requires a real file path rather than JSON or stdin. The default category
is `workflow` when `--node-code` is present, otherwise `general`. Supported
categories are `general`, `workflow`, `cost`, `quote`, `cpq`, `maintenance`,
`source`, and `backup`. Node codes and version codes must belong to the project.
Node names can change without losing attached documents. Repeated uploads never
overwrite earlier documents; reusing `--request-id` retries the same upload.
The upload result is already a compact file receipt; `--compact` is accepted
without reading the workspace. Download refuses an existing output unless
`--overwrite` is explicit.

Result kinds are `FileSettingsResult`, `ProjectFilesResult`, `ProjectFileResult`,
and `FileDownloadResult`; their strict schemas are discoverable through
`schema show --name command-envelope`. Settings and list responses include the
archive paths. Upload/download receipts contain the stored file ID and metadata;
downloads also include an output artifact path, byte size and SHA-256.

Applied `cost import` and `boq import` retain the exact source workbook with
category `source` in the `cost` folder; preview-only imports do not archive. Project-based cost, quote, CPQ,
maintenance and template workbook exports archive the exact generated bytes.
Template exports also retain the input template. Existing export and mutation
response shapes are unchanged; use `files list` to retrieve archive receipts.
Standalone `cost export --input ...` is not assigned to a project archive.

An archive failure is an error, never a successful export/import response.
If the output file or import was already saved, the error states that fact.
Recover with `files upload` using the retained source/output rather than
repeating an applied import and duplicating business rows. Archive writes do
not require reopening a locked cost version or completing a workflow node.
Ten global tabs include CPQ catalog and BU profit-share rates; legacy status remains compatibility data.
See [BU profit share and pricing](profit-share.md) for rate snapshots and target Sales GP.
