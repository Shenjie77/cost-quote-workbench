...
`manualCosts.otherServiceRate` is optional, numeric 0–1. Presence computes 2.3.4.2 from the sum of 2.3.1, 2.3.2 and 2.3.3; absence preserves manual `otherService`. Narrow cost settings patches that supply only `otherService` clear the rate, while an explicit rate keeps automatic mode. No migration changes historical inputs or manual amounts; automatic versions use the corrected base when read.
